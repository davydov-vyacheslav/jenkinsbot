...
public abstract class AbstractModifyBuildCommand implements BuildSubCommand, ProgressableCommand {
	...
	public void cancelProgress(Chat chat, User from) {
		...
		String fieldLabel = bot.getI18nMessage(from, "label.field.build." + state.getFieldKey());
		String currentAction = fieldLabel;
		if (!(state == StateType.NA_ADD || state == StateType.NA_EDIT)) {
			currentAction = bot.getI18nMessage(from, "message.command.build.cancel.field", new Object[] { fieldLabel });
		}
	...
	private InlineKeyboardMarkup buildRepoMarkup(User from) {
		...
		splitListByNElements(pageSize, fields)
				.forEach(fieldsValues -> inlineKeyboardMarkup.addRow(
						fieldsValues.stream()
								.map(fieldsValue -> new InlineKeyboardButton(bot.getI18nMessage(from, "button.build.setFieldValue", new Object[] { bot.getI18nMessage(from, "label.field.build." + fieldsValue.getFieldKey())}))
										.callbackData("/build " + getBuildType() + " " + fieldsValue.getFieldKey()))
								.toArray(InlineKeyboardButton[]::new)));

		inlineKeyboardMarkup.addRow(
				new InlineKeyboardButton(bot.getI18nMessage(from, "button.build.common.complete")).callbackData("/build " + getBuildType() + " " + ACTION_DONE),
				new InlineKeyboardButton(bot.getI18nMessage(from, "button.build.common.cancel")).callbackData("/cancel")
		);

		return inlineKeyboardMarkup;
	}

	public String getRepositoryDetails(User from, BuildInfoDto repo) {
		String fieldsInfo = Stream.of(StateType.REPO_NAME, StateType.PUBLIC, StateType.DOMAIN, StateType.USER, StateType.PASSWORD, StateType.JOB_NAME)
				.map(stateType -> "- "
						+ bot.getI18nMessage(from, "label.field.build." + stateType.getFieldKey())
						+ ": " + getOrIcon(stateType.getValue(repo)))
				.collect(Collectors.joining("\n"));

		return bot.getI18nMessage(from, "message.command.build.common.repoInfo.prefix", new Object[]{ fieldsInfo });
	}
